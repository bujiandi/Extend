...
//    internal init (_ handle:OpaquePointer!) { handle = handle }
//
...
//            if SQLITE_OK == sqlite3_prepare_v2(handle, "PRAGMA user_version", -1, &stmt, nil) {
//                defer { sqlite3_finalize(stmt) }
//                return SQLITE_ROW == sqlite3_step(stmt) ? Int(sqlite3_column_int(stmt, 0)) : 0
//            }
//            return -1
//        }
//        set { sqlite3_exec(handle, "PRAGMA user_version = \(newValue)", nil, nil, nil) }
//    }
//
...
//        _lastSQL = sql
//        let flag = sqlite3_exec(handle, sql, nil, nil, nil)
...
//        var stmt:OpaquePointer? = nil
//        _lastSQL = sql
//        if SQLITE_OK != sqlite3_prepare_v2(handle, sql, -1, &stmt, nil) {
...
//extension DBHandle {
//    // 单表查询
//    public func query<T>(_ sql:SQL<T>) throws -> DBResultSet<T> {
...
//        return sqlite3_exec(handle,"COMMIT TRANSACTION",nil,nil,nil)
//    }
//    // MARK: 回滚事务 ROLLBACK TRANSACTION
//    @discardableResult
//    func rollbackTransaction() -> CInt {
//        return sqlite3_exec(handle,"ROLLBACK TRANSACTION",nil,nil,nil)
//    }
//}
...
// MARK: - SQL构造器
public struct SQLHandle: CustomStringConvertible {
    
    fileprivate var sql: [String] = []
    
    public mutating func addCondition(_ condition:String, funcName:String = #function) {
        sql.append(String(funcName.split(separator: "(").first ?? ""))
...
public protocol DBSQLHandler {
    var handle:SQLHandle { get set }
    init(_ handle:SQLHandle?)
}

open class SQLBase: DBSQLHandler, CustomStringConvertible {
    
    open var handle:SQLHandle
    public required init(_ handle:SQLHandle? = nil) {
        self.handle = handle ?? SQLHandle()
    }
    
    @inlinable public var description: String { return handle.description }
}
...
// MARK: 2字段条件扩展运算符
public func <> <S, T1,T2>(lhs: T1, rhs: T2) -> DBCondition<S,T1,T2> {
...
// MARK: 以下是 条件无限追加的函数
public func <> <S, T1,T2>(lhs: DBCondition<S,T1,T2>, rhs: Any) -> DBCondition<S,T1,T2> {
...
open class SQLWhere<T:DBTableType>: SQLBase {
    ...
    public required init(_ handle:SQLHandle? = nil) { super.init(handle) }
    
    // MARK: where
    open func WHERE(_ column:T) -> Self {
        handle.addCondition("\(column)")
        return self
    }
    open func WHERE(_ condition:@autoclosure () -> DBCondition<SQLWhere,T,T>) -> Self {
        handle.addCondition(condition().description)
        return self
    }
    open var DESC:SQLWhere<T> {
        handle.sql.append("DESC")
        return self
    }
    open func ORDER(BY text:String) -> Self {
        handle.sql.append("ORDER BY \(text)")
        return self
    }
    open func ORDER(BY columns:T...) -> Self {
        handle.sql.append("ORDER BY " + columns.map({ "\($0)" }).joined(separator: ", "))
        return self
    }
    open func GROUP(BY columns:T...) -> Self {
        handle.sql.append("GROUP BY " + columns.map({ "\($0)" }).joined(separator: ", "))
        return self
    }
    open func LIMIT(_ value:Int) -> Self {
        handle.addCondition("\(value)")
        return self
    }
    open func LIKE(_ pattern:String) -> Self {
        handle.addCondition("'\(pattern)'")
        return self
    }
    open func BETWEEN(_ value1:Any, AND value2:Any) -> Self {
        handle.sql.append("BETWEEN")
        handle.sql.append(filterSQLAny(value1))
        handle.sql.append("AND")
        handle.sql.append(filterSQLAny(value2))
        return self
    }
    
    open func IS<N:DBNullType>(_ nullValue:N) -> Self {
        handle.addCondition(nullValue.description)
        return self
    }
...
//        handle.sql.append("IN(\(sql.description))")
//        return self
//    }
...
open class SQLBegan: SQLBase {
    ...
    fileprivate init(_ base:String) {
        super.init(SQLHandle())
        handle.sql.append(base)
    }
    
    open func INTO<Table:DBTableType>(_:Table.Type) -> SQLInsert<Table> {
        handle.addCondition(Table.table_name)
        return SQLInsert<Table>(handle)
    }
    
    open func TABLE<Table:DBTableType>(_:Table.Type) -> SQL<Table> {
        handle.addCondition(Table.table_name)
        return SQL<Table>(handle)
    }
    open func TABLE(_ oldTableName:String) -> Self {
        handle.addCondition(oldTableName)
        return self
    }
    open var RENAME:SQLBegan {
        handle.sql.append("RENAME")
        return self
    }
    ...
    open func FROM<T1:DBTableType, T2:DBTableType>(_:T1.Type,_:T2.Type) -> SQL2<T1,T2> {
        handle.sql.append("FROM \(T1.table_name), \(T2.table_name)")
        return SQL2<T1,T2>(handle)
    }
    
    open func COUNT(_ columns: (SQLBegan,SQLBegan)->SQLBegan) -> SQLBegan {
        handle.sql.append("COUNT(")
        return columns(self, SQLBegan(")"))
    }
    
    open var OR:SQLBegan {
        handle.sql.append("OR")
        return self
    }
    open var REPLACE:SQLBegan {
        handle.sql.append("REPLACE")
        return self
    }
    open var IGNORE:SQLBegan {
        handle.sql.append("IGNORE")
        return self
    }
...
open class SQL<T:DBTableType>:SQLWhere<T> {
    ...
    open func FROM(_:T.Type) -> Self {
        handle.addCondition(T.table_name)
        return self
    }
    
    // MARK: set
    open var SET:SQLSet<T> {
        handle.sql.append("SET")
        return SQLSet<T>(handle)
    }
    
    // MARK: select
    open func COUNT(_ columns:T...) -> Self {
        handle.sql.append("COUNT(" + columns.map({ "\($0)" }).joined(separator: ", ") + ")")
        return self
    }
    open func SELECT(_ columns:T...) -> Self {
        handle.sql.append("SELECT " + columns.map({ "\($0)" }).joined(separator: ", "))
        return self
    }
    open func SELECT(DISTINCT columns:T...) -> Self {
        handle.sql.append("SELECT DISTINCT " + columns.map({ "\($0)" }).joined(separator: ", "))
        return self
    }
    open func SELECT(TOP value:Int,_ columns:T...) -> Self {
        handle.sql.append("SELECT TOP \(value) " + columns.map({ "\($0)" }).joined(separator: ", "))
        return self
    }
    open var SELECT:SQL {
        handle.sql.append("SELECT")
        return self
    }
    
    // MARK: join
    open var LEFT:SQL {
        handle.sql.append("LEFT")
        return self
    }
    open var RIGHT:SQL {
        handle.sql.append("RIGHT")
        return self
    }
    open func JOIN<Table:DBTableType>(_:Table.Type) -> SQL2<T,Table> {
        handle.addCondition(Table.table_name)
        return SQL2<T,Table>(handle)
    }
    ...
    open func COLUMN(_ oldColumnName:String) -> Self {
        handle.addCondition(oldColumnName)
        return self
    }
    ...
    open var DROP:SQL {
        handle.sql.append("DROP")
        return self
    }
    open func MODIFY<Column:DBColumnTypeProtocol>(_ column:T,_ columnType:Column) -> Self {
        handle.sql.append("MODIFY \(column) \(columnType)")
        return self
    }
    open func ADD<Column:DBColumnTypeProtocol>(_ column:T,_ columnType:Column) -> Self {
        handle.sql.append("ADD \(column) \(columnType)")
        return self
    }
...
open class SQLSet<T:DBTableType>: SQLWhere<T> {
    ...
    public required init(_ handle:SQLHandle?) {
        super.init(handle)
    }
    
    open subscript(column:T) -> SQLSet<T> {
        if handle.sql.last != "SET" {
            handle.sql.append(",")
        }
        handle.sql.append("\(column)")
        return self
    }
    ...
    open subscript(condition:@autoclosure () -> DBCondition<SQLSet,T,T>) -> SQLSet<T> {
        ...
        handle.sql.append(condition().description)
        return self
    }
}


open class SQLInsert<T:DBTableType>: DBSQLHandler {
    open var handle:SQLHandle
    ...
    open func SELECT<Table:DBTableType>(_ columns:Table...) -> SQL<Table> {
        ...
        handle.addCondition(text)
        return SQL<Table>(handle)
    }
    
    open subscript(columns:T...) -> SQLInsert<T> {
        let text = columns.map({ "\($0)" }).joined(separator: ", ")
        handle.sql.append("(\(text))")
    ...
    open func VALUES(_ params:Any...) -> SQL<T> {
        ...
        handle.sql.append("VALUES(\(text))")
        return SQL<T>(handle)
    }
//    open func VALUES<U, BindSet>(_ values:[U], into db:DBHandle, binds:(_ id:Int64, _ value:U, _ bindSet:BindSet) throws -> () ) throws where BindSet : DBBindSet {
...
//        // TODO: 批量插入
//        //print(handle.sql.joinWithSeparator(" "))
//        let stmt = try db.query(handle.sql.joined(separator: " "))
//        //print(db.lastSQL)
//        // 方法完成后释放 数据操作句柄
//        //defer { sqlite3_finalize(stmt);print("释放插入句柄") }
//        let bindSet = DBBindSet<T>(stmt, columns)
//
//        db.beginTransaction { (transaction) in
...
//            // 不符合字段约束
//            throw NSError(domain: "Abort due to constraint violation", code: Int(flag), userInfo: ["sql":handle.sql.joined(separator: " ")])
//        }
//        sqlite3_reset(stmt)
//
//
...
open class SQL2<T1:DBTableType,T2:DBTableType>:SQL<T1> {
    ...
    open func ON(_ condition:@autoclosure () -> DBCondition<SQLWhere<T1>,T1,T2>) -> Self {
        handle.addCondition(condition().debugDescription)
        return self
    }
    
    // MARK: where
    open override func WHERE(_ column:T1) -> Self {
        handle.addCondition("\(T1.table_name).\(column)")
        return self
    }
    open func WHERE(_ column:T2) -> Self {
        handle.addCondition("\(T2.table_name).\(column)")
        return self
    }
...
//    }
    ...
    open func ORDER(BY columns:T2...) -> Self {
        handle.sql.append("ORDER BY " + columns.map({ "\(T2.table_name).\($0)" }).joined(separator: ", "))
        return self
    }
    open override func ORDER(BY columns:T1...) -> Self {
        handle.sql.append("ORDER BY " + columns.map({ "\(T1.table_name).\($0)" }).joined(separator: ", "))
        return self
    }
    ...
    open func GROUP(BY columns:T2...) -> Self {
        handle.sql.append("GROUP BY " + columns.map({ "\(T2.table_name).\($0)" }).joined(separator: ", "))
        return self
    }
    open override func GROUP(BY columns:T1...) -> Self {
        handle.sql.append("GROUP BY " + columns.map({ "\(T1.table_name).\($0)" }).joined(separator: ", "))
        return self
    }
    open override func LIMIT(_ value:Int) -> Self {
        _ = super.LIMIT(value)
...
// MARK: - 配合 SELECT 语句
public func *  <T>(lhs:SQLBegan, rhs:SQL<T>) -> SQL<T> {
    lhs.handle.sql.append("*")
    lhs.handle.sql.append(contentsOf: rhs.handle.sql)
    rhs.handle = lhs.handle
    return rhs
}
public func *  <T>(lhs:SQLBegan, rhs:SQLInsert<T>) -> SQLInsert<T> {
    lhs.handle.sql.append("*")
    lhs.handle.sql.append(contentsOf: rhs.handle.sql)
    rhs.handle = lhs.handle
    return rhs
...
public func SELECT(TOP value:Int) -> SQLBegan {
...
public func UPDATE<T:DBTableType>(_:T.Type) -> SQL<T> {
    ...
    sql.handle.sql.append(String(funcName.split(separator: "(").first ?? ""))
    sql.handle.sql.append(T.table_name)
    return sql
}
