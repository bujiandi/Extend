...
private func componentsFrom(path:String) -> [Substring] {
    ...
    if let index = path.firstIndex(of: "?") {
        components = path[..<index].split(separator: "/")
    } else {
...
public struct Path {
    ...
    public init(_ absolutePath:String) {
        components = componentsFrom(path: absolutePath).compactMap{ $0.isEmpty ? nil : String($0) }
    }
    
    public init(rootPath:Path, fileName:String) {
...
extension Path: ExpressibleByUnicodeScalarLiteral {
...
extension Path: ExpressibleByExtendedGraphemeClusterLiteral {
...
extension Path {
    ...
    ///   Swift.
    @inlinable public func shuffled<T>(using generator: inout T) -> Path where T : RandomNumberGenerator {
        return Path(components.shuffled(using: &generator))
    }
    ...
    ///
    /// - Complexity: O(*n*), where *n* is the length of the sequence.
    @inlinable public func shuffled() -> Path {
        return Path(components.shuffled())
    }
    ...
    @inlinable public static func + <Other>(lhs: Path, rhs: Other) -> Path where Other : Sequence, String == Other.Element {
    ...
    @inlinable public static func + <Other>(lhs: Other, rhs: Path) -> Path where Other : Sequence, String == Other.Element {
    ...
    ///   argument.
    @inlinable public static func += <Other>(lhs: inout Path, rhs: Other) where Other : Sequence, String == Other.Element {
    ...
    @inlinable public static func + <Other>(lhs: Path, rhs: Other) -> Path where Other : RangeReplaceableCollection, String == Other.Element {
        return Path(lhs.components + rhs.filter{ !$0.isEmpty })
    }
    ...
    @inlinable public func sorted(by areInIncreasingOrder: (String, String) throws -> Bool) rethrows -> Path {
        return Path(try components.sorted(by: areInIncreasingOrder))
    }
...
extension Path : CustomReflectable {
...
extension Path : RandomAccessCollection, MutableCollection, RangeReplaceableCollection {
